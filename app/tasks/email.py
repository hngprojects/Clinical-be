...
def send_otp_email_task(
    self, *, to_email: str, first_name: str, code: str, purpose: str
) -> None:
    try:
    ...
    try:
        send_otp_email(
            to_email=to_email, first_name=first_name, code=code, purpose=purpose_enum
        )
...
def send_password_reset_email_task(self, to_email: str, reset_token: str) -> None:
    try:
        ...
        logger.warning(
            "Password reset email task failed (retrying): %s", exc, exc_info=True
        )
        raise self.retry(exc=exc) from exc
