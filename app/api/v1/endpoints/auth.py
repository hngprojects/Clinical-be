...
async def signup(
    payload: SignupRequest, session: DBSession
...
async def verify_otp(
    payload: VerifyOtpRequest, session: DBSession
) -> SuccessResponse[TokenResponse]:
...
async def resend(
    payload: ResendOtpRequest, session: DBSession
) -> SuccessResponse[OtpDispatchResponse]:
...
async def forgot_password(
    request: ForgotPasswordRequest, session: DBSession
) -> SuccessResponse:
    ...
    if user:
        ...
        try:
            ...
            logger.exception(
                "Failed to enqueue password reset email for %s", _mask_email(user.email)
            )
    return SuccessResponse(
...
async def password_reset(
    request: ResetPasswordRequest, session: DBSession
) -> SuccessResponse:
